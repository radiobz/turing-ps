...
- **美颜**：磨皮、美白（GPUPixel WASM，Apache-2.0，本地 GPU 处理）
- **一键工具**：一键提亮（自动曝光）、一键打印、相框、抠图（背景色移除）、插画特效（素描/油画/马赛克/像素化/复古/黑白/反色）
- 基座能力：图层、滤镜、文字/艺术字、裁剪、素材库、模板、快捷键、右键菜单、历史撤销、PSD 导入

## 路线图

- [x] M1 工程骨架：基座整合（vue-fabric-editor）、品牌改造、构建打通
- [x] M2 编辑能力：图层、滤镜、文字、绘制、撤销/重做、美颜（GPUPixel WASM 磨皮/美白）
- [x] M3 部分：相框、插画特效、一键提亮、一键打印、抠图（浏览器端颜色抠图）
- [ ] M3 剩余：瘦脸/大眼/口红（GPUPixel 扩展编译）、rembg 深度抠图服务、装饰素材
- [ ] M4 发布：GitHub Pages 演示站点、PWA
